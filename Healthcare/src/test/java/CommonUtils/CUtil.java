...
public class CUtil {
	...
	public static void selectMultipleAnswers(By locator) {
		waitForElement(locator);
		List<WebElement> list = webDriver.findElements(locator);
		for (WebElement webElement : list) {
			javaScrollIntoView(webElement);
			try {
				Thread.sleep(1500);
			...
			javascript_highlight(webElement, "brown", "dotted", 3);
			actionClick(webElement);
		}
	}

	public static void selectMultipleAnswersNormal(By locator) {
		waitForElement(locator);
		List<WebElement> list = webDriver.findElements(locator);
		for (WebElement webElement : list) {
			javaScrollIntoView(webElement);
